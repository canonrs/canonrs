//! IconButton Behavior — data-rs-state only
...
pub fn register() {
    register_behavior("data-rs-icon-button", Box::new(|root: &web_sys::Element, _state: &ComponentState| -> BehaviorResult<()> {
        if root.get_attribute("data-rs-state").as_deref() == Some("disabled") {
            return Ok(());
        }

        if root.get_attribute("data-rs-loading").as_deref() == Some("loading") {
            add_state(root, "loading");
            return Ok(());
        }
