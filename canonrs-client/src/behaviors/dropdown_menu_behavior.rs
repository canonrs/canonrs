    ...
    register_behavior("data-rs-dropdown-menu", Box::new(|root: &web_sys::Element, _state: &ComponentState| -> BehaviorResult<()> {
        ...
        if let Ok(Some(trigger)) = root.query_selector("[data-rs-dropdown-menu-trigger]") {
            ...
            let cb = Closure::wrap(Box::new(move |_: MouseEvent| {
                ...
                let next = if !is_open { "open" } else { "closed" };
                dropdown_clone.set_attribute("data-rs-state", next).ok();
                if let Ok(Some(trigger_el)) = dropdown_clone.query_selector("[data-rs-dropdown-menu-trigger]") {
                    trigger_el.set_attribute("data-rs-state", next).ok();
                    trigger_el.set_attribute("aria-expanded", if next == "open" { "true" } else { "false" }).ok();
                }
                if !is_open {
                    if let Some(content) = dropdown_clone.query_selector("[data-rs-dropdown-menu-content]").ok().flatten() {
                        if let (Ok(c), Some(t)) = (content.dyn_into::<HtmlElement>(), trigger_clone.dyn_ref::<HtmlElement>()) {
                            ...
                            c.style().set_property("--dropdown-x", &format!("{}px", rect.left())).ok();
                            c.style().set_property("--dropdown-y", &format!("{}px", rect.bottom() + 4.0)).ok();
                        }
                    }
                }
            }) as Box<dyn FnMut(_)>);
