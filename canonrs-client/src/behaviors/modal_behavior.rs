...
#[cfg(feature = "hydrate")]
fn close_modal(root: &Element, trigger: &Option<Element>, open_signal: leptos::prelude::RwSignal<bool>, portal_id: String) {
...
pub fn register() {
    register_behavior("data-rs-modal", Box::new(|root: &web_sys::Element, state: &ComponentState| -> BehaviorResult<()> {
        ...
        let portal_id = get_portal_id(root, "modal");
        ...
        if let Some(ref t) = trigger {
            ...
            let cb = Closure::wrap(Box::new(move |_: MouseEvent| {
                ...
                if is_open {
                    close_modal(&root_clone, &trigger_clone, open_signal, portal_id_clone.clone());
                } else {
                    ...
                    mount_portal(&root_clone, &doc, &portal_id_clone, "[data-rs-modal-overlay]", "[data-rs-modal-content]");
                    ...
                    let cb_del = Closure::wrap(Box::new(move |e: MouseEvent| {
                        ...
                        let is_overlay = target.has_attribute("data-rs-modal-overlay") || target.closest("[data-rs-modal-overlay]").ok().flatten().is_some();
                        let is_close = target.has_attribute("data-rs-modal-close") || target.closest("[data-rs-modal-close]").ok().flatten().is_some();
                        if is_overlay || is_close {
                            close_modal(&root_del, &trigger_del, open_signal, pid.clone());
                        }
                    }) as Box<dyn FnMut(_)>);
        ...
        let cb_esc = Closure::wrap(Box::new(move |e: KeyboardEvent| {
            if e.key() == "Escape" && root_esc.get_attribute("data-rs-state").as_deref() == Some("open") {
                close_modal(&root_esc, &trigger_esc, open_signal, portal_id_esc.clone());
            }
        }) as Box<dyn FnMut(_)>);
