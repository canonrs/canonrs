...
#[cfg(feature = "hydrate")]
fn close_drawer(root: &web_sys::Element, trigger: &Option<web_sys::Element>, open_signal: leptos::prelude::RwSignal<bool>) {
    ...
    register_behavior("data-rs-drawer", Box::new(|root: &web_sys::Element, state: &ComponentState| -> BehaviorResult<()> {
        ...
        if let Some(ref t) = trigger {
            ...
            let cb = Closure::wrap(Box::new(move |_: MouseEvent| {
                ...
                if is_open {
                    close_drawer(&root_clone, &trigger_clone, open_signal);
                } else {
        ...
        if let Ok(Some(overlay)) = root.query_selector("[data-rs-drawer-overlay]") {
            let root_clone = root.clone();
            let trigger_clone = trigger.clone();
            let cb = Closure::wrap(Box::new(move |_: MouseEvent| {
                close_drawer(&root_clone, &trigger_clone, open_signal);
        ...
        if let Ok(close_btns) = root.query_selector_all("[data-rs-drawer-close]") {
        ...
        let cb_esc = Closure::wrap(Box::new(move |e: KeyboardEvent| {
            if e.key() == "Escape" && root_esc.get_attribute("data-rs-state").as_deref() == Some("open") {
                close_drawer(&root_esc, &trigger_esc, open_signal);
            }
        }) as Box<dyn FnMut(_)>);
