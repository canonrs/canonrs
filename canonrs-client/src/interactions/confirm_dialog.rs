...
pub fn init(root: Element) {
    {
        ...
        let cb = Closure::<dyn Fn(web_sys::MouseEvent)>::wrap(Box::new(move |e: web_sys::MouseEvent| {
            ...
            if target.closest("[data-rs-dialog-trigger]").ok().flatten().is_some() { open(&root_cb); return; }
            if target.closest("[data-rs-dialog-overlay]").ok().flatten().is_some() { close(&root_cb); return; }
