//! Modal Interaction — Canon Rule #342
...
pub fn init_all() {
    ...
    let nodes = match doc.query_selector_all("[data-rs-modal]") { Ok(n) => n, Err(_) => return };
    for i in 0..nodes.length() {
        if let Some(node) = nodes.item(i) {
            if let Ok(el) = node.dyn_into::<HtmlElement>() { init_modal(el); }
        }
    }
    bind_escape();
}

fn init_modal(root: HtmlElement) {
    // trigger → open
    if let Some(trigger) = root.query_selector("[data-rs-modal-trigger]").ok().flatten() {
    ...
    // overlay + close → close
    for selector in ["[data-rs-modal-overlay]", "[data-rs-modal-close]"] {
...
fn bind_escape() {
    let cb = Closure::<dyn Fn(_)>::wrap(Box::new(move |e: web_sys::KeyboardEvent| {
        ...
        let win = match web_sys::window() { Some(w) => w, None => return };
        let doc = match win.document() { Some(d) => d, None => return };
        let nodes = match doc.query_selector_all("[data-rs-modal][data-rs-state='open']") { Ok(n) => n, Err(_) => return };
        for i in 0..nodes.length() {
            if let Some(node) = nodes.item(i) {
