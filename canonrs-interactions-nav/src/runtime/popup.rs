...
//! Um único listener global por selector (não por instância)
...
pub fn register_click_outside(root_selector: &'static str, close_fn: fn(&Element)) {
    ...
    let cb = Closure::<dyn Fn(web_sys::MouseEvent)>::wrap(Box::new(move |e: web_sys::MouseEvent| {
        ...
        // se clicou dentro de qualquer root → ignora
        if target.closest(&selector).ok().flatten().is_some() { return; }
        ...
        for i in 0..nodes.length() {
            if let Some(root) = nodes.item(i).and_then(|n| n.dyn_into::<Element>().ok()) {
                if crate::runtime::state::has(&root, "open") { close_fn(&root); }
            }
        }
    }));
