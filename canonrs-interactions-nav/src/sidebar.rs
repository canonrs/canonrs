...
pub fn init(root: Element) {
    ...
    // sidebar search — esconde list até digitar
    if let Some(search) = query::first(&root, "[data-rs-sidebar-search]") {
        ...
        if let Some(input) = query::first(&search, "[data-rs-command-input]") {
            let search_cb = search.clone();
            let cb = Closure::<dyn Fn(web_sys::Event)>::new(move |e: web_sys::Event| {
                let value = e.target()
                    .and_then(|t| t.dyn_into::<web_sys::HtmlInputElement>().ok())
                    .map(|i| i.value())
                    .unwrap_or_default();
                if let Some(list) = query::first(&search_cb, "[data-rs-command-list]") {
                    if value.is_empty() {
                        let _ = list.set_attribute("style", "display:none;");
                    } else {
                        let _ = list.set_attribute("style", "display:block;");
                    }
                }
            });
    ...
    // sidebar group collapsible toggle
    {
        ...
        let cb = Closure::<dyn Fn(web_sys::MouseEvent)>::wrap(Box::new(move |e: web_sys::MouseEvent| {
            let Some(target) = query::safe_target(&e) else {
                return;
            };
            let toggle = target.closest("[data-rs-sidebar-group-toggle]").ok().flatten();
            let Some(toggle) = toggle else { return };
            e.stop_propagation();
            let group = toggle.closest("[data-rs-sidebar-group]").ok().flatten();
            let Some(group) = group else { return };
            web_sys::console::log_1(&format!("[sidebar-group] state before: {:?}", group.get_attribute("data-rs-state")).into());
            if state::is_expanded(&group) {
                state::collapse(&group);
            } else {
                state::expand(&group);
            }
            web_sys::console::log_1(&format!("[sidebar-group] state after: {:?}", group.get_attribute("data-rs-state")).into());
        }));
        let _ = root_cb2.add_event_listener_with_callback("click", cb.as_ref().unchecked_ref());
        cb.forget();
    }
