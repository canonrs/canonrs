...
pub fn init(root: Element) {
    {
        ...
        let cb = Closure::<dyn Fn(web_sys::MouseEvent)>::new(move |e: web_sys::MouseEvent| {
            ...
            if target.closest("[data-rs-sheet-trigger]").ok().flatten().is_none() { return; }
            open(&root_cb);
        });
...

    {
        ...
        let cb = Closure::<dyn Fn(web_sys::MouseEvent)>::new(move |e: web_sys::MouseEvent| {
            if !is_element_alive(&root_cb) { return; }
            let Some(target) = e.target().and_then(|t| t.dyn_into::<Element>().ok()) else { return };
            if target.closest("[data-rs-sheet-overlay]").ok().flatten().is_some() { close(&root_cb); }
            if target.closest("[data-rs-sheet-close]").ok().flatten().is_some() { close(&root_cb); }
        });
        let _ = root.add_event_listener_with_callback("click", cb.as_ref().unchecked_ref());
        cb.forget();
    }
