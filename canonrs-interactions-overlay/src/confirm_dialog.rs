...
pub fn init(root: Element) {
...

    {
        let cb = Closure::<dyn Fn(web_sys::MouseEvent)>::new(move |e: web_sys::MouseEvent| {
            ...
            if query::closest(&target, "[data-rs-confirm-dialog-trigger]") { open(&current); return; }
            if query::closest(&target, "[data-rs-confirm-dialog-overlay]") { close(&current); return; }
            if query::closest(&target, "[data-rs-confirm-dialog-cancel]")  { close(&current); return; }
            if query::closest(&target, "[data-rs-confirm-dialog-confirm]") { close(&current); }
        });
...

    {
        let cb = Closure::<dyn Fn(web_sys::KeyboardEvent)>::new(move |e: web_sys::KeyboardEvent| {
            ...
            query::each("[data-rs-confirm-dialog][data-rs-initialized='true']", |node| {
