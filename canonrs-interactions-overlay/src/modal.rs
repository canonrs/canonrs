...
fn open(root: &Element) {
    ...
    if let Ok(Some(content)) = root.query_selector("[data-rs-modal-content]") {
...
pub fn init(root: Element) {
...

    {
        let cb = Closure::<dyn Fn(web_sys::MouseEvent)>::new(move |e: web_sys::MouseEvent| {
            ...
            if query::closest(&target, "[data-rs-modal-trigger]") { open(&current); return; }
            if query::closest(&target, "[data-rs-modal-overlay]") { close(&current); return; }
            if query::closest(&target, "[data-rs-modal-close]")   { close(&current); }
        });
...

    {
        let cb = Closure::<dyn Fn(web_sys::KeyboardEvent)>::new(move |e: web_sys::KeyboardEvent| {
            ...
            query::each("[data-rs-modal][data-rs-initialized='true']", |node| {
