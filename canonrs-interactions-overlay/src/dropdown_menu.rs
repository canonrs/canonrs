...
//! Core: dom/lifecycle + Overlay: stack
...
fn set_open(root: &Element, open: bool) {
    ...
    if open { rem_tok(root, "closed"); add_tok(root, "open"); }
    else     { rem_tok(root, "open");  add_tok(root, "closed"); }
}
...
pub fn init(root: Element) {
...

    {
        listeners::listen(&uid, &root, "click", move |e: web_sys::Event| {
            ...
            let Some(cur) = me.current_target().and_then(|t| t.dyn_into::<Element>().ok()) else { return };
            let Some(t)   = me.target().and_then(|t| t.dyn_into::<Element>().ok()) else { return };
    ...
    {
        let uid2 = uid.clone();
        listeners::listen(&uid2, &root, "mouseover", move |e: web_sys::Event| {
            let Some(me) = e.dyn_into::<web_sys::MouseEvent>().ok() else { return };
            let Some(cur) = me.current_target().and_then(|t| t.dyn_into::<Element>().ok()) else { return };
            let Some(t)   = me.target().and_then(|t| t.dyn_into::<Element>().ok()) else { return };
            if let Ok(Some(item)) = t.closest("[data-rs-dropdown-menu-item]") {
    ...
    {
        let uid2 = uid.clone();
        listeners::listen(&uid2, &root, "keydown", move |e: web_sys::Event| {
            let Some(ke) = e.dyn_into::<web_sys::KeyboardEvent>().ok() else { return };
            let Some(cur) = ke.current_target().and_then(|t| t.dyn_into::<Element>().ok()) else { return };
            if !rs::is_open(&cur) { return; }
            match ke.key().as_str() {
                "Escape" | "Tab" => { rs::close(&cur); clear_focus(&cur); }
                "Enter" | " " => {
                    ke.prevent_default();
                    let items = navigable(&cur);
                    if let Some(idx) = focused_idx(&items) {
                        if let Some(el) = items.get(idx) {
                            if let Ok(h) = el.clone().dyn_into::<web_sys::HtmlElement>() { h.click(); }
                        }
                    }
                    rs::close(&cur); clear_focus(&cur);
                }
                "ArrowDown" => {
                    ke.prevent_default();
                    let items = navigable(&cur); let len = items.len(); if len == 0 { return; }
                    let next = match focused_idx(&items) { None => 0, Some(i) => (i+1).min(len-1) };
                    clear_focus(&cur); if let Some(el) = items.get(next) { focus_el(el); }
                }
                "ArrowUp" => {
                    ke.prevent_default();
                    let items = navigable(&cur); let len = items.len(); if len == 0 { return; }
                    let next = match focused_idx(&items) { None => len-1, Some(i) => if i==0 {0} else {i-1} };
                    clear_focus(&cur); if let Some(el) = items.get(next) { focus_el(el); }
                }
                _ => {}
            }
    ...
    {
        let uid2 = uid.clone();
        listeners::listen_document(&uid2, "click", move |e: web_sys::Event| {
            let Some(me) = e.dyn_into::<web_sys::MouseEvent>().ok() else { return };
            let Some(t)  = me.target().and_then(|t| t.dyn_into::<Element>().ok()) else { return };
