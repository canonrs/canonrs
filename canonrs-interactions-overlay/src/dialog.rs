...
pub fn init(root: Element) {
...

    {
        let cb = Closure::<dyn Fn(web_sys::MouseEvent)>::new(move |e: web_sys::MouseEvent| {
            ...
            if query::closest(&target, "[data-rs-dialog-trigger]") { open(&current); return; }
            if query::closest(&target, "[data-rs-dialog-overlay]") { close(&current); return; }
            if query::closest(&target, "[data-rs-dialog-close]")   { close(&current); }
        });
...

    {
        let cb = Closure::<dyn Fn(web_sys::KeyboardEvent)>::new(move |e: web_sys::KeyboardEvent| {
            ...
            query::each("[data-rs-dialog][data-rs-initialized='true']", |node| {
