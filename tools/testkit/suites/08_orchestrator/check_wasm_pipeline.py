...
def run():
    ...
    if not server_available(url):
        print(f"[SKIP] wasm_pipeline — server not running on {url}")
        return 0
    env = dict(os.environ)
    env["NODE_PATH"] = "/usr/lib/node_modules"
    env["CANON_BASE_URL"] = url
    try:
        result = subprocess.run(["node", os.path.join(DIR, "wasm_pipeline_tests.cjs")], env=env, timeout=180)
