#!/usr/bin/env python3
"""
check_hydration.py — Hydration Governance
Valida que SSR DOM == hydrated DOM via Playwright.

Garantias:
1. Todos os data-rs-interaction existem no SSR e no DOM hidratado
2. Todos os data-rs-uid existem no SSR e no DOM hidratado
3. Nenhum data-rs-* criado apenas pelo hydration (nao existia no SSR)
4. data-rs-state inicial e consistente entre SSR e hydration
5. Sem hydration mismatch no console
"""
import sys, subprocess, shutil, os, json

BASE_URL  = "http://localhost:3000"
SCRIPT    = os.path.join(os.path.dirname(os.path.abspath(__file__)), "hydration_tests.cjs")
GEN       = os.path.join(os.path.dirname(os.path.abspath(__file__)), "gen_hydration_tests.py")
...
const ROUTES = """ + json.dumps(ROUTES) + r""";
...
def run():
    if not shutil.which("playwright"):
        print("[SKIP] playwright nao encontrado")
        return 0
