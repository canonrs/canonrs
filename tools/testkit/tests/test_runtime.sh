...
echo "--- CanonRS Runtime Safety ---"

RESULT=$(python3 "$(dirname $0)/check_runtime.py" 2>&1)
ERRORS=$(echo "$RESULT" | grep "^\[FAIL\]" | grep -oP '\d+')
OK=$(echo "$RESULT" | grep "^\[OK\].*Runtime architecture canonical" | wc -l)

if [ -z "$ERRORS" ] || [ "$ERRORS" = "0" ]; then
    ok "$RESULT"
else
    echo "$RESULT" | grep "^\[ERRO\]" | while read line; do
        fail "runtime violation: $line"
    done
    fail "runtime governance: $ERRORS violations found"
fi
