...
#[cfg(feature = "hydrate")]
pub fn register() {
    register_behavior("data-tooltip", Box::new(|id: &str, state: &ComponentState| -> BehaviorResult<()> {
        use leptos::leptos_dom::helpers::document;

        let Some(root) = document().get_element_by_id(id) else {
            return Err(BehaviorError::ElementNotFound { selector: id.into() });
        };
        if root.get_attribute("data-tooltip-attached").as_deref() == Some("1") { return Ok(()); }
        root.set_attribute("data-tooltip-attached", "1").ok();

        let open_signal = state.open;
        let root_id = id.to_string();

        let Some(trigger) = document().query_selector(&format!("[data-tooltip-trigger=\'{}\']", root_id)).ok().flatten() else {
            return Ok(());
        };

        let content_sel = format!("#{} [data-tooltip-content]", root_id);
        let root_clone = root.clone();
        let doc_show = document();
        let content_sel_show = content_sel.clone();
        let trigger_sel = format!("[data-tooltip-trigger=\'{}\']", root_id).replace("\'", "'");
        ...
        let cb_show = Closure::wrap(Box::new(move |_: MouseEvent| {
            ...
            if let Some(content) = doc_show.query_selector(&content_sel_show).ok().flatten() {
                if let (Ok(c), Some(t)) = (content.dyn_into::<HtmlElement>(), doc_show.query_selector(&trigger_sel).ok().flatten().and_then(|el| el.dyn_into::<HtmlElement>().ok())) {
                    ...
                    c.style().set_property("transform", "translateX(-50%) translateY(-100%)").ok();
                }
            }
        }) as Box<dyn FnMut(_)>);
