...
pub fn init(table: &HtmlElement) {
...

    {
        ...
        let _ = head_el.add_event_listener_with_callback("mousedown", cb.as_ref().unchecked_ref());
        cb.forget();
    }

    {
        let drag_over_mm = drag_over.clone();
        let dragging_mm  = dragging.clone();
        let head_mm      = head_el.clone();
        let cb = Closure::<dyn Fn(web_sys::MouseEvent)>::wrap(Box::new(move |e: web_sys::MouseEvent| {
            if !dragging_mm.get() { return; }
            let Some(t) = e.target().and_then(|t| t.dyn_into::<web_sys::Element>().ok()) else { return };
            let Some(th) = t.closest("[data-rs-datatable-head-cell]").ok().flatten() else { return };
            let col_index = th.get_attribute("data-rs-col-index").and_then(|s| s.parse::<usize>().ok());
            if let Ok(cells) = head_mm.query_selector_all("[data-rs-drag-over]") {
            ...
            if let Some(idx) = col_index {
                drag_over_mm.set(Some(idx));
                let _ = th.set_attribute("data-rs-drag-over", "true");
            }
        }));
        let _ = web_sys::window().and_then(|w| {
            w.add_event_listener_with_callback("mousemove", cb.as_ref().unchecked_ref()).ok()
        });
        cb.forget();
    }

    {
        let drag_from_mu = drag_from.clone();
        let drag_over_mu = drag_over.clone();
        let dragging_mu  = dragging.clone();
        let head_mu      = head_el.clone();
        let table_mu     = table.clone();
        let cb = Closure::<dyn Fn(web_sys::MouseEvent)>::wrap(Box::new(move |_: web_sys::MouseEvent| {
            if !dragging_mu.get() { return; }
            dragging_mu.set(false);
            if let Ok(cells) = head_mu.query_selector_all("[data-rs-dragging],[data-rs-drag-over]") {
                for i in 0..cells.length() {
                    if let Some(el) = cells.item(i).and_then(|n| n.dyn_into::<web_sys::Element>().ok()) {
            ...
            let from = drag_from_mu.get();
            let to   = drag_over_mu.get();
            drag_from_mu.set(None);
            drag_over_mu.set(None);
            let (Some(from_idx), Some(to_idx)) = (from, to) else { return };
            if from_idx == to_idx { return; }
            reorder_columns(&table_mu, from_idx, to_idx);
            ...
            if let Ok(event) = web_sys::CustomEvent::new_with_event_init_dict("rs-datatable-col-reorder", &init) {
                let root: web_sys::Element = table_mu.clone().into();
                let _ = root.dispatch_event(&event);
            }
        }));
        let _ = web_sys::window().and_then(|w| {
            w.add_event_listener_with_callback("mouseup", cb.as_ref().unchecked_ref()).ok()
        });
        cb.forget();
    }
}
