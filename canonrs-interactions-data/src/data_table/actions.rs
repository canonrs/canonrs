...
pub fn init(table: &HtmlElement) {
    ...
    bind_row_actions(table, &uid);
    bind_bulk_actions(table, &uid);
}

fn bind_bulk_actions(table: &HtmlElement, uid: &str) {
    let root: web_sys::Element = table.clone().into();
    let ns = uid.to_string();
    listeners::listen(&ns, &root, "click", move |e: web_sys::Event| {
        use wasm_bindgen::JsCast;
...
fn bind_row_actions(_table: &HtmlElement, uid: &str) {
    let ns = uid.to_string();
    listeners::listen_document_capture(&ns, "click", move |e: web_sys::Event| {
        use wasm_bindgen::JsCast;
        let e = match e.dyn_into::<web_sys::MouseEvent>() { Ok(e) => e, Err(_) => return };
        {let e = e;
        let Some(t) = e.target().and_then(|t| t.dyn_into::<web_sys::Element>().ok()) else { return };
...
fn bind_context_menu(_table: &HtmlElement, uid: &str) {
    let ns = uid.to_string();
    let ns2 = uid.to_string();
    {
        listeners::listen_document(&ns, "contextmenu", move |e: web_sys::Event| {
        use wasm_bindgen::JsCast;
        let e = match e.dyn_into::<web_sys::MouseEvent>() { Ok(e) => e, Err(_) => return };
        {let e = e;
            let Some(t) = e.target().and_then(|t| t.dyn_into::<web_sys::Element>().ok()) else { return };
    ...
    {
        listeners::listen_document(&ns2, "click", move |_e: web_sys::Event| {
            let Some(win) = web_sys::window() else { return };
            let Some(d) = win.document() else { return };
...
fn bind_bulk_bar(table: &HtmlElement, uid: &str) {
    let root: web_sys::Element = table.clone().into();
    let ns = uid.to_string();
    let ns2 = uid.to_string();
    {
        listeners::listen(&ns, &root, "rs-selection-change", move |e: web_sys::Event| {
    ...
    if let Some(clear_btn) = root.query_selector("[data-rs-datatable-bulk-clear]").ok().flatten() {
        ...
        listeners::listen(&ns2, &clear_btn, "click", move |_e: web_sys::Event| {
