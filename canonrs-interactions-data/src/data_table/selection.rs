...
pub fn init(table: &HtmlElement) {
    ...
    {
        listeners::listen(&uid, &root, "mousedown", move |e: web_sys::Event| {
            use wasm_bindgen::JsCast;
    ...
    if let Some(select_all) = root.query_selector("[data-rs-datatable-select-all]").ok().flatten() {
        let uid2 = uid.clone();
        listeners::listen(&uid2, &select_all, "click", move |e: web_sys::Event| {
            use wasm_bindgen::JsCast;
            let e = match e.dyn_into::<web_sys::MouseEvent>() { Ok(e) => e, Err(_) => return };
    ...
    {
        let uid3 = uid.clone();
        listeners::listen(&uid3, &tbody, "keydown", move |e: web_sys::Event| {
            use wasm_bindgen::JsCast;
    ...
    {
        let uid4 = uid.clone();
        listeners::listen(&uid4, &tbody, "click", move |e: web_sys::Event| {
            use wasm_bindgen::JsCast;
            let e = match e.dyn_into::<web_sys::MouseEvent>() { Ok(e) => e, Err(_) => return };
            let Some(t) = e.target().and_then(|t| t.dyn_into::<web_sys::Element>().ok()) else { return };
