...
fn bind_legend(root: &Element, canvas: &HtmlCanvasElement, chart_type: &str, labels: &[String], series: &Series, show_grid: bool, show_legend: bool, height: f64) {
    ...
    for (i, (name, _, color, active)) in series.iter().enumerate() {
        ...
        let uid_legend = root_c.get_attribute("data-rs-uid").unwrap_or_default();
        listeners::listen(&uid_legend, &item, "click", move |_: web_sys::Event| {
...
fn bind_tooltip(root: &Element, canvas: &HtmlCanvasElement, chart_type: &str, labels: &[String], series: &Series, show_grid: bool, height: f64, sync_table: &Option<String>) {
    ...
    let on_move_fn = move |e: web_sys::MouseEvent| {
    ...
    let on_leave_fn = move |_: web_sys::MouseEvent| {
    ...
    let uid_tt = root.get_attribute("data-rs-uid").unwrap_or_default();
    let uid_tt2 = uid_tt.clone();
    listeners::listen(&uid_tt,  canvas.unchecked_ref(), "mousemove",  move |e: web_sys::Event| {
        use wasm_bindgen::JsCast;
        if let Ok(me) = e.dyn_into::<web_sys::MouseEvent>() { on_move_fn(me); }
    });
    listeners::listen(&uid_tt2, canvas.unchecked_ref(), "mouseleave", move |e: web_sys::Event| {
        use wasm_bindgen::JsCast;
        if let Ok(me) = e.dyn_into::<web_sys::MouseEvent>() { on_leave_fn(me); }
    });
}
...
fn bind_datatable_sync(root: &Element, canvas: &HtmlCanvasElement, chart_type: &str, labels: &[String], series: &Series, show_grid: bool, height: f64, sync_table: &Option<String>) {
    ...
    let canvas_c = canvas.clone(); let root_c = root.clone();
    let labels_c = labels.to_vec(); let series_c = series.clone(); let ct_c = chart_type.to_string();
    let on_hover_fn = move |e: web_sys::CustomEvent| {
        let detail = e.detail();
    ...
    let on_leave_fn = move |_e: web_sys::Event| {
        draw_chart(&canvas_c2, &ct_c2, &labels_c2, &series_c2, show_grid, height);
        if let Ok(Some(t)) = root_c2.query_selector("[data-rs-chart-tooltip]") { state::close(&t); }
        if let Ok(Some(c)) = root_c2.query_selector("[data-rs-chart-crosshair]") { state::close(&c); }
    };
    let uid_sync = root.get_attribute("data-rs-uid").unwrap_or_default();
    let uid_sync2 = uid_sync.clone();
    listeners::listen(&uid_sync,  root, "canon:datatable:hover", move |e: web_sys::Event| {
        use wasm_bindgen::JsCast;
        if let Ok(ce) = e.dyn_into::<web_sys::CustomEvent>() { on_hover_fn(ce); }
    });
    listeners::listen(&uid_sync2, root, "canon:datatable:leave", move |e: web_sys::Event| {
        on_leave_fn(e);
    });
}
