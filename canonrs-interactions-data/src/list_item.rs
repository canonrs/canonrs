...
pub fn init(root: Element) {
...

    {
        ...
        listeners::listen_uid(&root, "click", move |e: web_sys::Event| {
            use wasm_bindgen::JsCast;
...

    {
        ...
        listeners::listen_uid(&root, "keydown", move |e: web_sys::Event| {
            use wasm_bindgen::JsCast;
            let e = match e.dyn_into::<web_sys::KeyboardEvent>() { Ok(e) => e, Err(_) => return };
            let key = e.key();
            if !["ArrowUp", "ArrowDown", "Enter", " ", "Home", "End"].contains(&key.as_str()) { return; }
            let items = get_items(&root_kb);           if items.is_empty() { return; }
            let doc = match web_sys::window().and_then(|w| w.document()) {
                Some(d) => d, None => return,
            };
            let active = doc.active_element();
            let cur_idx = active.as_ref().and_then(|a| items.iter().position(|item| item == a));
            match key.as_str() {
                "ArrowDown" => {
                    e.prevent_default();
                ...
                "End"  => { e.prevent_default(); focus_item(&items, items.len() - 1); }
                "Enter" | " " => {
                    e.prevent_default();
