    ...
    view! {
        <FieldErrorPrimitive class=class>
            {if let Some(child) = children { child().into_any() }
             else { view! { {errors.into_iter().map(|e| view! { <span data-rs-field-error-item="">{e}</span> }).collect_view()} }.into_any() }}
        </FieldErrorPrimitive>
    }.into_any()
}
#[component]
pub fn FieldSeparator(#[prop(optional)] children: Option<Children>, #[prop(into, default = String::new())] class: String) -> impl IntoView {
    view! {
        <div data-rs-field-separator-wrapper="" class=class>
            ...
            {children.map(|c| view! { <span data-rs-field-separator-content="">{c()}</span> })}
        </div>
    }
}
